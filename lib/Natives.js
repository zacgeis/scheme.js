...
function _evalList(list, frame, evaluator, ret) {
  var evaled = [];
  var evaledCount = list.length;
  for(var i = 0; i < list.length; i++) {
    (function(i, item) {
      evaluator.eval(item, frame, function(result) {
        evaled[i] = result;
        evaledCount--;
        if(evaledCount === 0) {
          ret(new SValue('list', evaled));
        }
      });
    }(i, list[i]))
  }
...
/*
function _evalList(list, frame, evaluator, ret, isTailCall) {
  isTailCall = false;
  console.log('procname: ' + frame.resolve('__procname__'));
  console.log('isTailCall: ' + isTailCall + ' ' + list.length);

  if(list.length === 1) {
    evaluator.eval(list[0], frame,)
  }

  var inner = function(val) {
    _evalList(list.slice(1), frame, evaluator, function(result) {
  ...
  if(list.length === 0) {
    ...
    evaluator.eval(list[0], frame, isTailCall ? ret : inner);
  }
}
*/

/*
function _map(list, func, ret) {
  if(list.length === 0) {
    ret([]);
  } else {
    console.log('start', count++);
    func(list[0], function(val) {
      console.log('finished', count--);
      _map(list.slice(1), func, function(result) {
...
function _listToBegin(val) {
  return new SValue('array', [].concat(val.value));
}
...
defineProc('+', ['x', 'y'], ['number', 'number'], 'number', function(frame, evaluator) {
  var ret = frame.resolve('__ret__');
...
defineProc('append', ['target', 'value'], ['list', 'any'], 'list', function(frame, evaluator) {
  var ret = frame.resolve('__ret__');
...
defineProc('car', ['target'], ['list'], 'any', function(frame, evaluator) {
  var ret = frame.resolve('__ret__');
  var target = frame.resolve('target');
  ret(target.value[0]);
});
defineProc('cdr', ['target'], ['list'], 'list', function(frame, evaluator) {
  var ret = frame.resolve('__ret__');
  var target = frame.resolve('target');
...
defineMacro('if', ['cond', 'a', 'b'], ['expression', 'any', 'any'], 'any', function(frame, evaluator) {
  var ret = frame.previous.resolve('__ret__'); // if previous it works.
...
defineMacro('define', ['arg', '.', 'body'], ['any', 'any'], 'null', function(frame, evaluator) {
  ...
  if(arg.isExpression()) {
    ...
    var newproc = new SProc(name, procargs, [], '', _listToBegin(body), frame.previous);
...
defineMacro('eval', ['body'], ['expression'], 'null', function(frame, evaluator) {
  var ret = frame.previous.resolve('__ret__');
  var body = frame.resolve('body');
...
defineMacro('begin', ['.', 'body'], ['expression'], 'null', function(frame, evaluator, ret) {
  var body = frame.resolve('body');
  _map(body.value, function(val, mapRet) {
    //evaluator.eval(val, frame.previous, function(evalResult) { // change this.
    //  mapRet(evalResult);
    //});
    evaluator.eval(val, frame.previous, mapRet);
  }, function(result) {
...
defineMacro('begin', ['.', 'body'], ['expression'], 'null', function(frame, evaluator) {
  ...
  if(body.value.length === 1) {
    evaluator.eval(body.value[0], frame.previous, ret);
  }
  _evalList(body.value, frame.previous, evaluator, function(result) {
    //console.log(ret);
    ret(result.value[result.value.length - 1]);
  }, false);
});
...
defineMacro('let', ['arg', '.', 'body'], ['any'], 'null', function(frame, evaluator) {
  ...
  evaluator.eval(_listToBegin(body), frame, ret);
});
defineMacro('lambda', ['arg', '.', 'body'], ['any'], 'any', function(frame, evaluator) {
  ...
  var newproc = new SProc('anonymous', arg, [], '', _listToBegin(body), frame.previous);
  var newval = new SValue('proc', newproc);
  ret(newval);
});
