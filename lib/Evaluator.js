function initNatives(root) {
  ...
  root.set('begin', new Macro(['.', 'body'], function(context, evaluator) {
    return evaluator.eval(context.lookup('body'), context.parent);
  }));
  root.set('let', new Macro(['arg', '.', 'body'], function(context, evaluator) {
    var arg = context.lookup('arg');
    var body = context.lookup('body');
    arg = arg.value.map(function(token) {
      var name = token.value[0].value;
      var value = token.value[1].value;
      context.set(name, value);
      return name;
    });
    return evaluator.eval(body, context);
  }));
  root.set('lambda', new Macro(['arg', '.', 'body'], function(context, evaluator) {
    var arg = context.lookup('arg');
    var body = context.lookup('body');
    arg = arg.value.map(function(token) {
      return token.value;
    });
    return new Func(arg, body);
  }));
